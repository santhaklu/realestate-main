...
//Student Info
app.get('/studentinfo/submit', async function (req, res) {
    try {
        const conn = await client.connect();
        const db = conn.db('s12');
...
app.get('/markinfo/submit', async function (req, res) {
    try {
        const conn = await client.connect();
        const db = conn.db('s12');
        const mark = db.collection('mark');
        const data = await mark.find({}).toArray();
        conn.close();
        res.json(data);
    } catch (err) {
        res.json(err).status(404);
...
app.post('/sendemail',async function(req,res){
    try{
        var transport=nodemailer.createTransport({
            ...
            port:445,
            secure:true,
            auth:{user:"dhanunjayp4@gmail.com",pass:"wxpzahhomtwjmogu"}

        });
        // var emaildata={
        //     from:"dhanunjayp4@gmail.com",
        //     to:"dhanunjayp67@gmail.com",
